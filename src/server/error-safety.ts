...
const SAFE_APPROVAL_REASON = "approval_already_granted_but_not_visible";
...
function safeRemoteDetails(code: ErrorCode, details: Record<string, unknown> | undefined): Record<string, unknown> | undefined {
  if (code !== ErrorCode.APPROVAL_REQUIRED || !details) return undefined;
  ...
  if (details.reason === SAFE_APPROVAL_REASON) out.reason = SAFE_APPROVAL_REASON;
